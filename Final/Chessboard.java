...
public class Chessboard implements Comparable<Chessboard> {
	...
	// Chessboard(a);
	// Generate a board using an int array
	Chessboard(int[] a){
		l = new ArrayList<Point>();
		n = a.length;
	...
	// FindBestMoves(N,c);
	// Trims the least optimal movements from a provided ArrayList<Point>
	ArrayList<Point> findBestMoves(ArrayList<Point> N,int c){
		int bestH=-1;
		int curH;
		Point curPos=l.get(c);
		/*
		System.out.println("Available Positions: ");
		for(int i=0; i<N.size(); i++){
			System.out.println("("+N.get(i).x+","+N.get(i).x+")");
		}
		*/

		// Finding the H of the best options
		for(int i=0; i<N.size(); i++){
			l.set(c,N.get(i));
			if(bestH==-1){
				bestH = getHeuristic();
		...
		// Removing options less than the best options
		for(int i=0; i<N.size(); i++){
			l.set(c,N.get(i));
			if(getHeuristic() > bestH){
				N.remove(i);
			}
			l.set(c,curPos);
		}
		/*
		System.out.println("Available Positions Post-Trimming: ");
		for(int i=0; i<N.size(); i++){
			System.out.println("("+N.get(i).x+","+N.get(i).x+")");
		}
		*/
